...
datapath = '/home/alsjur/PhD/Kode/test_data/'

scales = np.geomspace(20000,150000,num=10,dtype=int)
iterations = [25]#np.arange(1,15)


# load data
dsU = xr.open_dataset(datapath+'LLC2160_U_Arctic_day_0000_k021_snapshot.nc').squeeze()
dsV = xr.open_dataset(datapath+'LLC2160_V_Arctic_day_0000_k021_snapshot.nc').squeeze()
...
meanpi = grid.average(ds_gcm.energy_transfer, ['X','Y'])
#meanpii = grid.average(dspii.energy_transfer, ['X','Y'])
meanpic = gridc.average(ds_gcmc.energy_transfer, ['X','Y'])

meane = grid.average(ds_gcm.energy, ['X','Y'])
#meanei = grid.average(dspii.energy, ['X','Y'])
meanec = gridc.average(ds_gcmc.energy, ['X','Y'])
...
for i, n in enumerate(iterations):
    ...
    pi = meanpi.sel(n_iterations=n)
    e = meane.sel(n_iterations=n)
    ax1.plot(scales/1e3,pi, marker='x', label=f'{n} iterations', color=colors[i])
    ax2.plot(scales/1e3,e, marker='x', color=colors[i])
    
    
    pic = meanpic.sel(n_iterations=n)
    ec = meanec.sel(n_iterations=n)
    ax1.plot(scales[3:]/1e3,pic, ls='--', color=colors[i])
    ax2.plot(scales[3:]/1e3,ec, ls='--', color=colors[i])
    
    

#ax1.plot(ls*1.5,meanpii, marker='x', label='scipy', color='black')
#ax2.plot(ls*1.5,meanei, marker='x', color='black')
#axes[0].set_yscale('log')
#ax.set_xscale('log')
#ax.set_ylim(-1.5e-10,0)
#ax1.set_xlim(0,300)
ax1.set_ylim(-2.5e-10,2e-10)
ax2.set_ylim(0,3e-3)


ax1.legend(ncol = 4)

fig.savefig('compare_iterations.png')